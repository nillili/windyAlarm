...
- **장기 스레드**: N분마다 폴링, 13시 전이면 오늘~+6일, 13시 이후면 +1~+6일 예보 출력
...
**장기 스레드 (lterm)**
- `lterm`분마다 항상 실행
- 출력 대상: 13시 전이면 오늘~+6일, 13시 이후면 +1~+6일, 각 날 13:00 KST 기준
- 출력 내용:
...
main.py
  ...
  └── lterm 스레드 (daemon=True)  ← sterm 시작 5초 후 시작
        └── loop: fetch → (13시 전: 오늘~+6일 / 13시 이후: +1~+6일) 예보 출력
                  → sleep(lterm분)
```
